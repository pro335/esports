    ...
    render() {
        ...
        for(let i = 0 ; i < category.length; i++){
            ...
            row = {
                ...
                status:category[i].status ?<button onClick={()=>{this.handleCloseRow(category[i]._id)}} style={{padding:"0px"}} className="btn btn-secondary btn-sm btn-delete mb-0 b-r-4" style={{paddingLeft:"15px",paddingRight:"15px"}}>{"ACTIVE"}</button>:<button onClick={()=>{this.handleCloseRow(category[i]._id)}} className="btn btn-primary btn-sm btn-delete mb-0 b-r-4">{"INACTIVE"}</button>,
                        ...
                        <div>
                            ...
                            <span onClick={() => {
                                if (window.confirm('Are you sure you wish to delete this vendor?')) {
                                    let data = category;let num = row.index;
